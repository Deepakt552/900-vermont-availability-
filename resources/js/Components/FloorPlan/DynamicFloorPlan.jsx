...
export default function DynamicFloorPlan({ onUnitSelect, filters }) {
    ...
    // Generate unit data based on the selected floor
    const generateUnitsForFloor = (floorNumber) => {
        // Generate unit numbers based on floor (e.g., floor 2 = 201-239, floor 3 = 301-339, etc.)
        ...
        const unitTypes = [
            {
                ...
                bathrooms: 1,
                area_sqft: 450,
                amenities: ['In-unit laundry', 'Modern kitchen', 'City views', 'Hardwood floors', 'High ceilings']
            },
            {
                name: '1 Bedroom',
                bedrooms: 1,
            ...
            },
            {
                ...
                bathrooms: 2.5,
                area_sqft: 1200,
                amenities: ['In-unit laundry', 'Modern kitchen', 'Balcony', 'City views', 'Walk-in closet', 'Den', 'Master suite', 'Private terrace']
            }
        ];

        return unitNumbers.map((unitNumber, index) => {
            const unitType = unitTypes[index % unitTypes.length];
            const basePrice = floorNumber > 4 ? 200 : 0; // Higher floors cost more
    ...
    useEffect(() => {
        if (svgContent && svgRef.current) {
            ...
            textElements.forEach((textElement, index) => {
                ...
                console.log(`Processing text element ${index}: "${unitNumber}"`);
                
                // Check if this is a unit number for the current floor (3 digits starting with the floor number)
                ...
                if (isUnitNumber) {
                    ...
                    console.log(`Making unit ${unitNumber} interactive, status: ${unit.status}`);
                    
                    try {
                        // Create invisible clickable area over the text
                        const bbox = textElement.getBBox();
                        const clickArea = document.createElementNS('http://www.w3.org/2000/svg', 'rect');
                        
                        // Make clickable area larger than text for better UX
                        const padding = 20;
                        ...
                        clickArea.setAttribute('data-unit-status', unit.status);
                        clickArea.setAttribute('rx', '8');
                        
                        // Style the text based on availability and search
                        ...
                        if (unit.status === 'available' && !isFiltered && matchesSearch) {
                            // Available units - green background with hover effects
                            ...
                            clickArea.addEventListener('click', (e) => {
                                e.preventDefault();
                                e.stopPropagation();
                                console.log('Unit clicked:', unit.unit_number);
                                handleUnitClick(unit);
                            });
                            
                        } else if (unit.status === 'available' && (isFiltered || !matchesSearch)) {
                            // Filtered out available units - dimmed
                            clickArea.setAttribute('fill', 'rgba(16, 185, 129, 0.1)');
                            clickArea.setAttribute('stroke', '#10B981');
                            ...
                            // Occupied units - subtle styling
                            clickArea.setAttribute('fill', 'rgba(107, 114, 128, 0.1)');
                            clickArea.setAttribute('stroke', '#9CA3AF');
                            clickArea.setAttribute('stroke-width', '1');
                            clickArea.style.opacity = matchesSearch ? '1' : '0.6';
                            textElement.style.fill = '#6B7280';
                            textElement.style.fontSize = '25px';
                        }
                        
                        // Insert the clickable area before the text element
            ...
            if (searchTerm) {
                ...
                console.log('Highlighting search results:', matchingUnits.length);
                matchingUnits.forEach(unit => {
                    const clickArea = svgElement.querySelector(`rect[data-unit-number="${unit.unit_number}"]`);
                    if (clickArea && unit.status === 'available') {
                        clickArea.setAttribute('fill', 'rgba(245, 158, 11, 0.4)');
                        clickArea.setAttribute('stroke', '#F59E0B');
                        clickArea.setAttribute('stroke-width', '3');
                    }
                });
            }
            
            console.log('SVG processing complete. Available units:', units.filter(u => u.status === 'available').length);
        }
            ...
            <div className="bg-white rounded-lg shadow-lg overflow-hidden">
                <div className="relative bg-gradient-to-br from-gray-50 to-gray-100 overflow-hidden flex items-center justify-center" style={{ minHeight: '600px' }}>
                    <div className="text-center">
                        ...
                        <p className="text-gray-500 text-sm mt-2">Unable to load floor plan layout</p>
                    </div>
                </div>
            </div>
        );
        ...
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
            ...
            <div className="bg-gradient-to-r from-gray-50 to-white border-t border-gray-200 p-6">
                <div className="flex items-center justify-center space-x-12">
                    ...
                    <div className="flex items-center group">
                        <div className="w-5 h-5 bg-amber-400 border border-amber-500 rounded-sm mr-3 shadow-sm"></div>
                        <span className="text-sm font-semibold text-gray-700 group-hover:text-amber-600 transition-colors">Search Results</span>
                    </div>
                </div>

                <div className="text-center mt-4 pt-4 border-t border-gray-100">
                    <p className="text-xs text-gray-500 font-medium">
                        Floor {selectedFloor} • James M Wood Boulevard • {availableUnits.length} Available Units • Click green units for details
                    </p>
                    <p className="text-xs text-gray-400 mt-1">
                        Hover over unit numbers to see details • Use search to find specific units • Switch floors using buttons above
                    </p>
                </div>
            </div>
